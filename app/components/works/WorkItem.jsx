            ...
            <div className="flex max-md:flex-col max-md:items-center max-md:justify-center">
              <div
                className={`overflow-hidden ${
                  id % 2 === 0 ? "order-1" : "order-0"
                } bg-back w-[85vw] h-72 rounded-[50px] 2xl:w-[900px] 2xl:h-[550px] md:w-[750px] md:h-[450px] 2xl:rounded-[75px] md:rounded-[70px]`}
              >
                <Image
                  src={src}
                  alt="weatherApp"
                ...
                <div className="flex justify-center">
                  <div className=" flex flex-col h-40 rounded-3xl w-[85vw] max-md:py-8 max-md:px-1 bottom-28 justify-evenly bg-back absolute drop-shadow-2xl md:bottom-9  2xl:w-[820px] 2xl:h-[220px] md:rounded-[45px] md:w-[670px] md:h-[200px] md:px-6 2xl:px-8">
                    <p className="  text-white text-sm md:text-lg 2xl:text-xl 2xl:leading-[50px] md:leading-10">
                      {p}
                      <br />
                      {KeyLesson.map(({ text, bgColor, index }) => (
                        <>
                          <span
                            key={index}
                            className={`p-1 rounded-md text-base text-[#fff] ${bgColor}`}
                          >
                            {`${text}`}
                          </span>
                          <span> </span>
              ...
              <h2
                className={`md:self-end pb-10 mt-10 self-center text-4xl ${
                  id % 2 === 0 ? "md:pr-10" : "md:pl-10"
                }`}
              >
                {h2}
              </h2>
